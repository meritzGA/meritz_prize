...
if mode == "⚙️ 관리자 모드":
    ...
    for i, cfg in enumerate(st.session_state['config']):
        ...
        with col1:
            ...
            st.info("💡 매니저 및 동명이인 식별을 위해 아래 4개 컬럼을 정확히 지정해주세요.")
            ...
            cfg['col_manager'] = st.selectbox("지원매니저명 컬럼", cols, index=get_idx(cfg['col_manager'], cols), key=f"cmanager_{i}")
...
# ==========================================
    ...
    st.markdown("<h3 style='color:#191f28; font-weight:800; font-size:1.3rem; margin-bottom: 15px;'>매니저 이름을 입력하세요.</h3>", unsafe_allow_html=True)
    ...
    if submit_manager:
        if not manager_name:
            st.warning("매니저 이름을 입력해주세요.")
        elif not st.session_state['config']:
            st.warning("현재 진행 중인 시책 데이터가 없습니다.")
        else:
            has_data = False
            for i, cfg in enumerate(st.session_state['config']):
                if 'col_manager' not in cfg or not cfg['col_manager']:
                    continue
                    
                df = st.session_state['raw_data'].get(cfg['file'])
                if df is not None:
                    search_manager = df[cfg['col_manager']].fillna('').astype(str).str.strip()
                    match_df = df[search_manager == manager_name.strip()]
                    ...
                    if not match_df.empty:
                        ...
                        for _, row in match_df.iterrows():
                            ...
                            try:
                                if "1기간" in p_type or "2기간" in p_type:
                                    val = float(str(row[cfg['col_val_curr']]).replace(',', ''))
                                else:
                                    val = float(str(row[cfg['col_val']]).replace(',', ''))
                            except:
                                val = 0.0
                            ...
                            # 다음 구간이 존재하는 사람만 목록에 추가
                            if shortfall > 0:
                                agent_list.append({
                                    '소속지점': agent_branch,
                                    '설계사명': agent_name,
                                    '현재실적': int(val),
                                    '도달구간': int(tier_achieved),
                                    '다음구간': int(next_tier) if next_tier else 0,
                                    '부족금액': int(shortfall)
                                })
                        
                        if agent_list:
                            res_df = pd.DataFrame(agent_list)
                            # 부족금액이 적은 사람(근접자)부터 위로 정렬
                            res_df = res_df.sort_values(by='부족금액')
                            ...
                            # 15만원 이하로 부족한 사람은 🚨 이모지로 특별 강조
                            res_df['부족금액'] = res_df['부족금액'].apply(lambda x: f"🚨 {x:,.0f}원 부족!" if x <= 150000 else f"{x:,.0f}원")
                            
                            st.dataframe(res_df, use_container_width=True, hide_index=True)
                        else:
                            st.info("해당 시책은 산하 설계사 모두 최고 구간을 달성했습니다.")
                            
            if not has_data:
                st.error("일치하는 매니저 정보가 없거나, 관리자 설정에서 '지원매니저명 컬럼'이 지정되지 않았습니다.")
...
# ==========================================
    ...
    # 🌟 HTML 박스 완전 삭제로 빈 하얀 박스 버그 100% 차단 🌟
